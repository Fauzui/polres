...
if (isset($_POST['tambahuser'])) {
	...
	if ($row > 0) {
		...
		$sql = mysqli_query($connect, "INSERT INTO administrator VALUES ('', '$email', '$nama', '$username', '$password_hash')");
    ...
    <form class="text-left" action="" method="POST">
        ...
        <fieldset class="border p-2">
            ...
            <div class="form-group">
                <label>Email</label>
                <input type="text" value="<?php if (isset($_GET['act']) && $_GET['act'] =='edit') { echo $email; } ?>"
                    name="email" class="form-control col-6" placeholder="Email Address" required>
                <label>Nama User</label>
                <input type="text" value="<?php if (isset($_GET['act']) && $_GET['act'] =='edit') { echo $nama; } ?>"
                    name="nama" class="form-control col-6" placeholder="Nama Lengkap" required>
                <label>Username</label>
                <input type="text"
                    value="<?php if (isset($_GET['act']) && $_GET['act'] =='edit') { echo $username; } ?>"
                    name="username" class="form-control col-6" placeholder="Username" required>
                <label>Password</label>
                <input type="password"
                    value="<?php if (isset($_GET['act']) && $_GET['act'] =='edit') { echo $password; } ?>"
                ...
                <button class="btn btn-lg btn-primary btn-block text-uppercase mt-4 col-2" type="submit"
                    name="<?=(isset($id) ? 'edituser' : 'tambahuser')?>"> <?=(isset($id) ? 'EDIT' : 'TAMBAH')?>
                </button>
            ...
            <tbody>
		  	...
		  	?>
                ...
                <tr>
                    ...
                    <td> <a href="?mod=useradmin&act=edit&id=<?php echo $row['ID']; ?>">Ubah</a> / <a
                            href="?mod=useradmin&act=hapus&id=<?php echo $row['ID']; ?>">Hapus</a></td>
                </tr>
                <?php $i++; ?>
                <?php endwhile; ?>
            </tbody>
